...
class Course(models.Model):
    # Replace direct User relation with user_id
    owner_id = models.PositiveIntegerField()  # Store user ID from Auth service
    subject = models.ForeignKey(Subject, related_name="courses", on_delete=models.CASCADE)
    title = models.CharField(max_length=200)
    overview = models.TextField()
    slug = models.CharField(max_length=200, unique=True)
    created = models.DateTimeField(auto_now_add=True)
    students = models.ManyToManyField('Student', related_name="courses_joined", blank=True)  # Use custom Student model
